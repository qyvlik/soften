#include "characterencodingconverter.h"

const unsigned int
CharacterEncodingConverter::GB2312ToUnicodeTable[][2] =
{
    0xA1A1, 0x3000,  /* '　' -> 12288 */
    0xA1A2, 0x3001,  /* '、' -> 12289 */
    0xA1A3, 0x3002,  /* '。' -> 12290 */
...
const unsigned int
CharacterEncodingConverter::UnicodeToGB2312Table[][2] =
{
    0x00A4,0xA1E8,
    0x00A7,0xA1EC,
    0x00A8,0xA1A7,
...
CharacterEncodingConverter::U16
CharacterEncodingConverter::UnicodeToGB2312(CharacterEncodingConverter::U16 unicode)//用二分查找算法
{
    CharacterEncodingConverter::U16 mid, low, high, len;
    len = sizeof(UnicodeToGB2312Table)/sizeof(UnicodeToGB2312Table[0]);
    low = 0;
    high = len - 1;

    if( unicode < 0x00A4) return unicode;

    while(low <= high) {
        mid = (low + high) / 2;
        if(UnicodeToGB2312Table[mid][0] > unicode)
            high = mid - 1;
        else if(UnicodeToGB2312Table[mid][0] < unicode)
            low =  mid + 1;
        else
            return 	UnicodeToGB2312Table[mid][1];
    }
    return 0 ; //没找到
}


CharacterEncodingConverter::U16 CharacterEncodingConverter::GB2312ToUnicode(CharacterEncodingConverter::U16 gb2312)
{

    CharacterEncodingConverter::U16 mid, low, high, len;
    len = sizeof(GB2312ToUnicodeTable)/sizeof(GB2312ToUnicodeTable[0]);
    low = 0;
    high = len - 1;

    if( gb2312 < 0xA1A1) return gb2312;

    while(low <= high) {
        mid = (low + high) / 2;
        if(GB2312ToUnicodeTable[mid][0] > gb2312)
            high = mid - 1;
        else if(GB2312ToUnicodeTable[mid][0] < gb2312)
            low =  mid + 1;
        else
            return GB2312ToUnicodeTable[mid][1];
    }
    return 0 ; //没找到
}
